...
clockInBtn.addEventListener('click', async () => {
  try {
    ...
    document.getElementById('openBadge').innerText = 'Yes';
  } catch(e) { window.App.toasts(e.message || 'Clock in failed') }
});
...
clockOutBtn.addEventListener('click', async () => {
  ...
  } catch(e){ window.App.toasts(e.message || 'Clock out failed') }
});
...
submitShift.addEventListener('click', async () => {
  ...
  try {
    // Call the EU-deployed callable function finalizeShiftEU
    const finalize = httpsCallable(functions, 'finalizeShiftEU');
    const res = await finalize({ shiftId: currentShiftId });
    await updateDoc(doc(db,'shifts',currentShiftId), { notes, links: links, closedAt: serverTimestamp(), status: 'closed', earnings: res.data?.earnings ?? 0 });
...
function watchShifts(uid){
  ...
  onSnapshot(q, snap => {
    ...
    snap.forEach(d => {
      ...
      const inT = s.clockIn ? new Date((s.clockIn.seconds || s.clockIn._seconds) * 1000).toLocaleString() : '-';
      const outT = s.clockOut ? new Date((s.clockOut.seconds || s.clockOut._seconds) * 1000).toLocaleString() : (s.status==='open' ? 'open' : '-');
...
async function computeTotals(uid){
  ...
  snap.forEach(d=>{
    ...
    const closed = (s.closedAt.toDate) ? s.closedAt.toDate() : new Date((s.closedAt.seconds || s.closedAt._seconds) * 1000);
